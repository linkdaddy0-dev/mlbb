...
function run() {
  ...
  if (!fs.existsSync(MANIFEST_PATH)) {
    console.error("❌ Error: manifest.json not found! Please run node scripts/mirror_assets.js first.");
    process.exit(1);
  }
  ...
  console.log("[1/3] Rewriting all compiled patch guide JSON files...");
  rewriteDirectory(PATCHES_DIR, manifest);

  // 2. Rewrite src/App.jsx hardcoded URLs
  console.log("\n[2/3] Rewriting hardcoded spells, emblems, and talent URLs in App.jsx...");
  ...
  // 3. Clean up SmartImage.jsx proxying logic completely
  console.log("\n[3/3] Deleting obsolete proxy interceptors inside SmartImage.jsx...");
  if (fs.existsSync(SMART_IMAGE_PATH)) {
    let smartImageContent = fs.readFileSync(SMART_IMAGE_PATH, 'utf-8');
    
    // Complete refactoring to perform zero proxying and load local assets directly
